...
// Every user has name and socket_id
...
int findUser(User **users,char *user_name,int userNum);
        ...
        for(socket=0;socket<fdmax;socket++){
            if(FD_ISSET(socket,&read_fds)){
                ...
                if(recv_size==-1){
                    ...
                    handleRequest(function,basic_message,&master,fdmax,server_socket,users,socket,&userNum);
...
void handlePrivate(User **users, int send_to, char *basic_message, int userNum){
    ...
    strcpy(dest_name,users[destid]->user_name);
...
int findUser(User **users, char *mes_body, int id){
    int i=0;
    for(i=0;i<id;i++){
        if(strcmp(users[i]->user_name,mes_body)==0){
            return 1;
        }
    }
    return 0;
...
int checkcommand(char * recv_msg,char * msg){
    ...
    if(strcmp(command,"broadcast")||strcmp(command,"Broadcast")){
        type=BROADCAST;
    }
    else 
    if(strcmp(command,"private")||strcmp(command,"Private")){
        type=PRIVATE;
    }
    command=strtok(NULL," ");
    while (command!=NULL){
